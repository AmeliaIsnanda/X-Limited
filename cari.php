<?php
    ...
    $sql = mysqli_query($koneksi, 'select * from sepatu where nama like \'%' . $_GET['cari'] . '%\'');
?>
					...
					<div class="row">
						<?php while ($sepatu = mysqli_fetch_assoc($sql)): ?>
							<div class="col-md-6 text-left">
								...
								<a href="/PROJECTPBW/detail.php?id=<?= $sepatu['id'] ?>" class="btn btn-primary">Detail</a> Rp.<?= number_format($sepatu['price'], 0, ',', '.') ?>,-
							</div>
						<?php endwhile ?>
					</div>
				</div>
